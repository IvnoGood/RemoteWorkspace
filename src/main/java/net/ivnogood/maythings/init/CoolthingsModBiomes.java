...
import net.ivnogood.maythings.world.biome.SuperbioleBiome;
...
public class CoolthingsModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, CoolthingsMod.MODID);
	public static final RegistryObject<Biome> SUPERBIOLE = REGISTRY.register("superbiole", () -> SuperbioleBiome.createBiome());

	@SubscribeEvent
	public static void init(FMLCommonSetupEvent event) {
		event.enqueueWork(() -> {
			SuperbioleBiome.init();
		});
	}
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
						...
						parameters.add(new Pair<>(SuperbioleBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, SUPERBIOLE.getId()))));
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							...
							surfaceRules.add(1,
									preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, SUPERBIOLE.getId()),
											CoolthingsModBlocks.GENERATIONBLOCK.get().defaultBlockState(),
											CoolthingsModBlocks.SUPERBLOCK.get().defaultBlockState(),
											CoolthingsModBlocks.SUPERBLOCK.get().defaultBlockState()));
